...
class CountryModel extends Model
{
	 ...
	 * @return false | array 省份信息
	 */
	public function getAllCountry($order = "letter ASC")
	{
	 ...
	 * @return false | array 城市信息
	 */
	public function getStateByParentid($countryid = 0,$order = "letter ASC")
	{
		$countryid = intval($countryid);
		if($countryid <= 0) return false;
		return M(self::$state_table)->where(array('countryid'=>$countryid))->order($order)->select();
	}
	/**
	 * 根据stateid查询地区
	 ...
	 * @return false | array 城市信息
	 */
	public function getCityByParentid($stateid = 0,$order = "letter ASC")
	{
		$stateid = intval($stateid);
		if($stateid <= 0) return false;
		return M(self::$city_table)->where(array('stateid'=>$stateid))->order($order)->select();
	}
