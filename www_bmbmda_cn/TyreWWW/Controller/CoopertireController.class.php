...
class CoopertireController extends Controller {
    private $brandid = 69;
    private $brand_name = 'uniroyal';
    private $catid = 0;
    private $seriesid = 0;
    private $seriesid_other;
    private $series_name = '';
    private $companyid = '37';
    private $company_name = 'uniroyal';
    private $thumb = '';
    private $org_thumb = '';
    private $thumb2 = '';
    private $thumb3 = '';
    ...
    public function index()
    {
      ...
      /*  $data['catname'] = $catname;
        ...
        $series_arr = array();
        $series_arr = explode('||',$series_name);
        foreach ($series_arr as $key => $value) {
          ...
          $tmp = explode('=&gt;',$value);
           $data['source_url'] = $source_url;
           $data['catname'] = $tmp[0];
           $data['series_name'] = $tmp[1];
           $data['series_content'] = $tmp[2];
           M('save_data')->add($data);
        }
        ...
        foreach ($model_arr_tr[1] as $key_tr => $value_tr) {
            ...
            if(!empty($value_tr))
            {
                ...
                //型号
                  ...
                  $XL = 0;
                  $model_arr_td_2 = '';
                  $model_arr_td_2 = $model_arr_td[1][0].'/'.$model_arr_td[1][1].'R'.$model_arr_td[1][2];
                 ...
                 /* if(preg_match('/SSR/', $model_arr_td[1][7]))
                  ...
                  {
                    $this->addGoodSpec(array('燃油效率'=>$model_arr_td[1][6]));
                  }

                  if(!empty($model_arr_td[1][7]))
                  {
                    $this->addGoodSpec(array('制动'=>$model_arr_td[1][7]));
                  }

                  if(!empty($model_arr_td[1][8]))
                  {
                    $this->addGoodSpec(array('舒适'=>$model_arr_td[1][8]));
                  }
                  ...
                  if(!empty($model_arr_td[1][6]))
                  {
                    ...
                    {
                      $utqg_arr[] = $model_arr_td[1][9];
                    }
                    if(!empty($model_arr_td[1][10]))
                    {
                      $utqg_arr[] = $model_arr_td[1][10];
                    }
                    if(!empty($model_arr_td[1][11]))
                    {
                      $utqg_arr[] = $model_arr_td[1][11];
                    }
                    $utqg_str = '';
                    if(!empty($utqg_arr))
                    {
                       $utqg_str = implode(' ', $utqg_arr);

                    }
                    $this->addGoodSpec(array('UTQG'=>$utqg_str));

                     if(!empty($model_arr_td[1][12]))
                  {
                    $zhuanshu = '';
                    $zhuanshu = $model_arr_td[1][12];
                   ...
                   if(!empty($model_arr_td[1][13]))
                  {
                    $dantai = '';
                    $dantai_arr = array();
                    $dantai_arr = explode('@',$model_arr_td[1][13]);
                   ...
                   if(!empty($model_arr_td[1][14]))
                  {
                    $shuangtai = '';
                    $shuangtai_arr = array();
                    $shuangtai_arr = explode('@',$model_arr_td[1][14]);
                  ...
                  {
                    $zhijing = '';
                    $zhijing = $model_arr_td[1][15];
                    $zhijing = $zhijing *25.4;
                    $this->addGoodSpec(array('轮胎外直径(mm)'=>$zhijing));
                  }


                  if(!empty($model_arr_td[1][16]))
                  {
                    $this->addGoodSpec(array('允许使用轮辋'=>$model_arr_td[1][16]));
                  }

                  if(!empty($model_arr_td[1][17]))
                  {
                    $fanwei = '';
                    $fanwei = $model_arr_td[1][17];
