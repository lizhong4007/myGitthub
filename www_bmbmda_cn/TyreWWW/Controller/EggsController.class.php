...
class EggsController extends Controller {
    private $brandid = 47;
    private $brand_name = '百路驰';
    private $catid = 0;
    private $seriesid = 2075;
    //型号存放路径
    private $filetext = "F:\\bfgoodrich/r1.txt";
    private $site = 'https://shop.bfgoodrichtires.com/tire/bf-goodrich/rugged-trail-t-a';

    private $seriesid_other;
    private $series_name = '';
    private $companyid = '33';
    private $company_name = '百路驰';
    private $thumb = '';
    private $org_thumb = '';
    private $thumb2 = '';
    private $thumb3 = '';
...
// exit;
        ...
        /*操作步骤：先将excel复制放入text中，转换编码为utf-8*/
       $seriesid = $series_info['seriesid'];
        $data = array();
        foreach ($arr as $key => $value) {
          $model_arr_td = array();
          $model_arr_td = explode("\t",$value);
          // print_r($model_arr_td);exit;
          if(count($model_arr_td) != 14 and count($model_arr_td) > 1)
          {
            echo 12;die;
          }
                  ...
                  if(!empty($model_arr_td_2))
                  {
                    ...
                    if(preg_match('/ST/', $model_arr_td_2))
                    {
                       $model_arr_td_2 = preg_replace('/\/ST/', '', $model_arr_td_2);
                       $model_arr_td_2 = preg_replace('/ST/', '', $model_arr_td_2);
                    }
                  }
                  $model_arr_td_3 = $model_arr_td_2;
                  ...
                  $model_arr_td_3 = preg_replace('/\/E/', '', $model_arr_td_3);
                  $model_arr_td_3 = preg_replace('/\/D/', '', $model_arr_td_3);
                  $model_arr_td_3 = preg_replace('/ST/', '', $model_arr_td_3);
                  $model_arr_td_3 = preg_replace('/\/LL/', '', $model_arr_td_3);
                  ...
                  $is_innertyre = 0;
                  /*if(preg_match('/TL/', $model_arr_td[1][8]))
                  {
                    $is_innertyre = 1;
                  }

                  if(preg_match('/TT/', $model_arr_td[1][8]))
                  {
                    $is_innertyre = 2;
                  }*/

                  $language = 1;//语种默认0中文,1英文
                  ...
                  $bian_str = '';
                  $bian_str = $model_arr_td_3_arr[1];
                  if(!empty($sudu))
                  {
                     $bian_str = preg_replace('/[^0-9\/]/','',$bian_str);
                     $fuzhong_tmp = array();
                     $fuzhong_tmp = explode('/',$bian_str);
                     if(empty($fuzhong_tmp[1]))
                     {
                      $bian_str = $fuzhong_tmp[0];
                     }
                     $this->addGoodSpec(array('负重指数'=>$bian_str));
                  }

                  $sudu = '';
                  $sudu = $model_arr_td_3_arr[2];
                  if(!empty($sudu))
                  {
                    if(preg_match('/PR/'))
                    {
                  ...
                  if(!empty($model_arr_td[3]))
                  {
                    $this->addGoodSpec(array('胎面特征'=>$model_arr_td[3]));

                  }

                  if(!empty($model_arr_td[4]))
                  {
                    $this->addGoodSpec(array('侧壁'=>$model_arr_td[4]));

                  }

                  if(!empty($model_arr_td[6]))
                  {
                    $fanhui = '';
                    $fanhui = $model_arr_td[6];
                    $fanhui = trim($fanhui);
                    $fanhui = preg_replace('/[a-zA-Z]/','-',$fanhui);
                    $fanhui = preg_replace('/(-)+/','-',$fanhui);
                    $fanhui = preg_replace('/[^0-9\.\-]/','',$fanhui);
                    $this->addGoodSpec(array('允许使用轮辋'=>$fanhui));
                  }
                  ...
                  if(!empty($biao))
                  {
                    $biao = preg_replace('/MT/','',$biao);
                    $biao_tmp = array();
                    $biao_tmp = explode('on',$biao);
                    $biao = array_pop($biao_tmp);
                    $lunwan_v = '';
                    $lunwan_v = $biao;
                  ...
                  if(!empty($model_arr_td[8]))
                  {
                    $zhijing = '';
                    $zhijing = $model_arr_td[8];
                    $zhijing = $zhijing*25.399999961392;
                    $zhijing =  number_format($zhijing,0);
                    $this->addGoodSpec(array('轮胎外直径(mm)'=>$zhijing));
                  }

                  if(!empty($model_arr_td[9]))
                  {
                    $this->addGoodSpec(array('转速/英里'=>$model_arr_td[9]));

                  }

                   if(!empty($model_arr_td[10]))
                  {
                    $this->addGoodSpec(array('花纹深度(mm)'=>$model_arr_td[10]));
                  }

                  if(!empty($model_arr_td[11]))
                  {
                    $max_fuhe = '';
                    $max_fuhe = $model_arr_td[11];
                    $max_fuhe_arr = array();
                    $max_fuhe_arr = explode('@',$max_fuhe);
                    $max_fuhe = array_shift($max_fuhe_arr);
                    $max_fuhe = number_format($max_fuhe/0.45359237038038,2);
                   

                    $this->addGoodSpec(array('最大负荷能力(单胎)'=>$max_fuhe));
                  }

                  if(!empty($model_arr_td[12]))
                  {
                    $max_fuhe1 = '';
                    $max_fuhe1 = $model_arr_td[12];
                    $max_fuhe_arr1 = array();
                    $max_fuhe_arr1 = explode('@',$max_fuhe1);
                    $max_fuhe1 = array_shift($max_fuhe_arr1);
                    $max_fuhe1 = number_format($max_fuhe1/0.45359237038038,2);
                   

                    $this->addGoodSpec(array('最大负荷能力(双胎)'=>$max_fuhe1));
                  }


                   if(!empty($model_arr_td[13]))
                  {
                    $dkuan = '';
                    $dkuan = $model_arr_td[13];
                    /*$dkuan = $dkuan*25.399999961392;
                    $dkuan =  number_format($dkuan,0);*/
                    $dkuan = number_format($dkuan/0.45359237038038,2);
                    $this->addGoodSpec(array('轮胎重(kg)'=>$dkuan));
                  }
...
public function addSeries($createdata = array()){
  ...
  public function add_series_content($content,$language = 0)
  {
    ...
    if(!empty($info))
    {
      M('series_content')->where(array('scid'=>$info['scid']))->save(array('content'=>$content));
    }else{
...
function img_download($url,$path="./Images/",$type = ''){
    ...
    // $header[] = "Host:www.wanli-global.com";
