...
class Brand extends MY_Controller {
	...
	public function brand_detail()
	{
		...
		$total_rows = $this->db->from('series')->where($where)->count_all_results();
		...
		if($current_page - 1 > 1)
		{
			$prev_link = site_url('brand/brand_detail/'.$brand_param.'/'.$brand['linkurl'].'/'.($current_page - 1));
		}
		...
		{
			$next_link = site_url('brand/brand_detail/'.$brand_param.'/'.$brand['linkurl'].'/'.($current_page + 1));
		}
		$data['next_link'] = $next_link;

		if(!empty($current_page))
		{
			$url = site_url('brand/brand_detail/'.$brand_param.'/'.$brand['linkurl'].'/'.$current_page);
		}else{
