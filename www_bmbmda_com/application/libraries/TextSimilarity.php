...
Class TextSimilarity {
	...
	private function stopwords(){
		$stopwords = file_get_contents("./stopwords-en.txt");
		$this->_excludeArr = explode("\n",$stopwords);
	}
    /**
     * [外部调用]
     *
    ...
    public function segment($text)
    {
		...
		$seg_text = preg_split("/[\pZ\pC]+/u",$text,null,PREG_SPLIT_NO_EMPTY);
		
		$stemmer = new PorterStemmer();
		foreach($seg_text as $s){
			$word[] = $stemmer->stem($s);
		}
