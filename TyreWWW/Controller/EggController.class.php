...
class CoopertireController extends Controller {
    ...
    private $seriesid = 0;
    //型号存放路径
    private $filetext = 'F:\www\wwwmotor\application\controllers\tyre.txt';
    ...
    public function index()
    {
        ...
        foreach ($arr as $key => $value) {
          ...
          foreach ($tmp as $k => $model_arr_td) {
                  ...
                  if(!empty($model_arr_td[2]))
                  {
                    $this->addGoodSpec(array('胎面特征'=>$model_arr_td[2]));

                  }

                  if(!empty($model_arr_td[3]))
                  {
                    $this->addGoodSpec(array('侧壁'=>$model_arr_td[3]));

                  }

                  if(!empty($model_arr_td[5]))
                  {
                    $fanhui = '';
                    $fanhui = $model_arr_td[5];
                    $fanhui = trim($fanhui);
                  ...
                  if(!empty($model_arr_td[7]))
                  {
                    $zhijing = '';
                    $zhijing = $model_arr_td[7];
                  ...
                  {
                    $this->addGoodSpec(array('转速/英里'=>$model_arr_td[8]));

                  }

                   if(!empty($model_arr_td[9]))
                  {
                    $this->addGoodSpec(array('花纹深度(mm)'=>$model_arr_td[9]));
                  }

                  if(!empty($model_arr_td[10]))
                  {
                    $max_fuhe = '';
                    $max_fuhe = $model_arr_td[10];
                    $max_fuhe_arr = array();
                    ...
                    $this->addGoodSpec(array('最大负荷(kg)'=>$max_fuhe));
                  }
