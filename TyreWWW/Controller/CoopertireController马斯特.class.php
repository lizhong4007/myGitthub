...
class CoopertireController extends Controller {
    private $brandid = 29;
    private $brand_name = '诺记';
    private $catid = 0;
    private $seriesid = 0;
    private $seriesid_other;
    private $series_name = '';
    private $companyid = '18';
    private $company_name = '诺记轮胎';
    private $thumb = '';
    private $org_thumb = '';
    private $thumb2 = '';
    private $thumb3 = '';
    ...
    public function index()
    {
        ...
        $thumb_arr0 = array();
        $thumb_arr0 = img_download($thumb_arr_tmp[0],'./Images/','series');
        ...
        {   $thumb_arr1 = array();
            $thumb_arr1 = img_download($thumb_arr_tmp[1],'./Images/','series');
            if($thumb_arr1['code'] != 0)
            {
                $this->thumb2 = $thumb_arr1['imagePath'];
            }
        }

        if(!empty($thumb_arr_tmp[2]))
        {   $thumb_arr2 = array();
            $thumb_arr2 = img_download($thumb_arr_tmp[2],'./Images/','series');
            if($thumb_arr2['code'] != 0)
            {
                $this->thumb3 = $thumb_arr2['imagePath'];
            }
        }
        ...
        $series_data_tmp = array('series_name'=>$series_name,'series_alias'=>$series_name);
      ...
      $seriesid = $this->seriesid;
        ...
        foreach ($model_arr_tr[1] as $key_tr => $value_tr) {
            ...
            if(!empty($value_tr))
            {
                ...
                //型号
                  ...
                  $is_innertyre = 0;
                  /*if(preg_match('/TL/', $model_arr_td[1][13]))
                  {
                    $is_innertyre = 1;
                  }

                  if(preg_match('/TT/', $model_arr_td[1][13]))
                  {
                    $is_innertyre = 2;
                  }*/
                  ...
                  {
                    $this->addGoodSpec(array('允许使用轮辋'=>$model_arr_td[1][3]));
                  }

                  if(!empty($model_arr_td[1][4]))
                  {
                    $this->addGoodSpec(array('断面宽度'=>$model_arr_td[1][4]));
                  }
                  if(!empty($model_arr_td[1][5]))
                  {
                    $this->addGoodSpec(array('轮胎外直径(mm)'=>$model_arr_td[1][5]));
                  }
       ...
       //车型数据
        ...
        {
          $cars_arr_tr = array();
          preg_match_all("/&lt;tr&gt;(.*?)&lt;\/tr&gt;/", $cars, $cars_arr_tr);
          foreach ($cars_arr_tr[1] as $key_tr => $value_tr) {
              $cars_arr_td = array();
              if(!empty($value_tr))
              {
                  preg_match_all("/&lt;td&gt;(.*?)&lt;\/td&gt;/", $value_tr, $cars_arr_td);
                  
                  $carsids = array();
                  $tmp = array();
                  $tmp = explode('、',$cars_arr_td[1][3]);
                  ...
                  $model_cars = array();
                  $model_cars['model_name'] = trim($cars_arr_td[1][0]);
