...
class CoopertireController extends Controller {
    private $brandid = 11;
    private $brand_name = '玛吉斯';
    private $catid = 22;
    private $seriesid = 168;
    private $seriesid_other;
    private $series_name = 'MAZ1';
    private $companyid = '8';
    private $company_name = '玛吉斯';
    private $thumb = '/Images/thumb/2016/11/03/1478174293_1341.png';
    private $org_thumb = '/Images/series/2016/11/03/1478174293_1341.png';
      ...
      $seriesid = $this->seriesid;
        ...
        foreach ($model_arr_tr[1] as $key_tr => $value_tr) {
            ...
            if(!empty($value_tr))
            {
                ...
                $model_arr_td[1][0] = preg_replace('/LT/', '', $model_arr_td[1][0]);
                $model_arr_td[1][0] = preg_replace('/P/', '', $model_arr_td[1][0]);
                ...
                $XL = 0;
                  ...
                  //产品
                  $is_left = 0;
                  if(preg_match('/左轮/', $model_arr_td[1][1]))
                  {
                    $model_arr_td[1][1] = preg_replace('/左轮/', '', $model_arr_td[1][1]);
                    $is_left = 1;
                  }
                  if(preg_match('/右轮/', $model_arr_td[1][1]))
                  {
                    $model_arr_td[1][1] = preg_replace('/右轮/', '', $model_arr_td[1][1]);
                    $is_left = 2;
                  }
                  $goods = array();
                  $goods['is_enhance'] = $XL;
                  $goods['is_left'] = $is_left;
                  $this->addGoods($goods);

                   //参数
                  $fuhe = $model_arr_td[1][1];//负荷
                  ...
                  $this->addGoodSpec(array('速度级别'=>$weight));

                  //添加默认参数
                  $this->addDefaultParam('负重指数',$fuhe);
                  $this->addDefaultParam('速度级别',$weight);
        ...
        if(!empty($cars))
        {
          ...
          foreach ($cars_arr_tr[1] as $key_tr => $value_tr) {
              ...
              if(!empty($value_tr))
              {
                  ...
                  $carsids = '';
                  $carsids = $this->addCars($cars_arr_td[1][1]);
                  $model_cars = array();
