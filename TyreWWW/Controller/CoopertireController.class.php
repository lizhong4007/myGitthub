...
class CoopertireController extends Controller {
    private $brandid = 34;
    private $brand_name = 'mickey thompson';
    private $catid = 0;
    private $seriesid = 0;
    private $seriesid_other;
    private $series_name = '';
    private $companyid = '31';
    private $company_name = 'mickey thompson';
    private $thumb = '';
    private $org_thumb = '';
    private $thumb2 = '';
    private $thumb3 = '';
    ...
    public function index()
    {
        ...
        $series_content = preg_replace('/( )/', ' ', $series_content);
        $series_content = trim($series_content);
        $language = 1;//语种，默认0中文，1英文
        $this->add_series_content($series_content,$language);
      ...
      $seriesid = $this->seriesid;
        ...
        foreach ($model_arr_tr[1] as $key_tr => $value_tr) {
            ...
            if(!empty($value_tr))
            {
                ...
                if(count($model_arr_td[1]) != 11)
                {
                  echo 9 ;die;
                }

                /*if(!preg_match('/PR/',$model_arr_td[1][1]))
                {
                  echo 'PR';die;
                }*/
                //型号
                  ...
                  $XL = 0;
                  $model_arr_td_2 = '';
                  $model_arr_td_2 = $model_arr_td[1][2];
                  ...
                  {
                     $lianbu = '';
                     $lianbu = preg_replace('/PR/','',$model_arr_td[1][1]);
                     
                     $houdu = '';
                     $houdu = preg_replace('/[^a-zA-Z]/','',$lianbu);

                     $lianbu = preg_replace('/[^0-9]/','',$lianbu);

                     $this->addGoodSpec(array('厚度'=>$LR));
                  ...
                  /* if(!empty($model_arr_td[1][3]))
                  {
                    ...
                    $this->addGoodSpec(array('LUG'=>$model_arr_td[1][3]));
                  }
                  ...
                  // 2：前轮：1后轮
                  /*$lunwei = '';
                  $lunwei = $model_arr_td[1][0];
                  $lunwei_v = 1;
                  if(preg_match('/front/',$lunwei))
                  {
                    $lunwei_v = 2;

                  }
                  $this->addGoodSpec(array('轮位'=>$lunwei_v));
                  */

                 /* $sudu = '';
                  $sudu = $model_arr_td[1][4];
                  if(!empty($sudu))
                  {
                    if(preg_match('/PR/'))
                    {
...
*/
                 ...
                 /* if(!empty($model_arr_td[1][4]))
                  {
                    $this->addGoodSpec(array('螺栓圈'=>$model_arr_td[1][4]));

                  }*/
                 /* if(!empty($model_arr_td[1][7]))
                  {
                    $this->addGoodSpec(array('负重等级'=>$model_arr_td[1][7]));
                 ...
                 /**/
                  ...
                  {
                    $this->addGoodSpec(array('向后间距'=>$model_arr_td[1][5]));
                  }
                  if(!empty($model_arr_td[1][6]))
                  {
                    $this->addGoodSpec(array('胶印(mm)'=>$model_arr_td[1][6]));
                  }
                   if(!empty($model_arr_td[1][7]))
                  {
                    $this->addGoodSpec(array('轮毂孔(in)'=>$model_arr_td[1][7]));
                  }
                  if(!empty($model_arr_td[1][8]))
                  {
                    $fuhe = '';
                    $fuhe = $model_arr_td[1][8];
                    $fuhe = number_format($fuhe/0.45359237038038,2);
                   

                    $this->addGoodSpec(array('最大负荷(kg)'=>$fuhe));
                  }
/*
                  if(!empty($model_arr_td[1][4]))
                  {
                    $this->addGoodSpec(array('允许使用轮辋'=>$model_arr_td[1][4]));
                  }*/

                  /*$biao = '';
                  $biao = $model_arr_td[1][5];
                  if(!empty($biao))
                  {
                  ...
                  {
                    $dkuan = '';
                    $dkuan = $model_arr_td[1][7];
                    $dkuan = $dkuan*25.399999961392;
                    $dkuan =  number_format($dkuan,0);
                    $this->addGoodSpec(array('断面宽度'=>$dkuan));
                  }

                   if(!empty($model_arr_td[1][8]))
                  {
                    $zhijing = '';
                    $zhijing = $model_arr_td[1][8];
                  ...
                  /*   if(!empty($model_arr_td[1][8]))
                  {
                    $this->addGoodSpec(array('圆周'=>$model_arr_td[1][8]));
                  }
*/
                  //    if(!empty($model_arr_td[1][15]))
                  // {
                  //   $this->addGoodSpec(array('花纹深度(mm)'=>$model_arr_td[1][15]));
                  // }
                  ...
                  //   $this->addGoodSpec(array('最大充气压力(kPa)'=>$model_arr_td[1][8]));
                  // }
